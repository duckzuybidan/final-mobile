...
export default function UserSlot({
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      const requestRef = collection(db, "friend_requests");
      await setDoc(
        doc(requestRef, `${currentEmail}-${userEmail}`),
        {
          from: currentEmail,
          to: userEmail,
  ...
  const handleInvite = async (toEmail: string) => {
    ...
    try {
      ...
      setDoc(
        doc(invitationRef, `${currentEmail}-${toEmail}-${id}`),
        {
          from: currentEmail,
          to: toEmail,
          roomID: id,
  ...
  const handleCardPlay = async () => {
    ...
    const filteredHand = player.hand.filter(
    ...
    const fetchFriends = async () => {
      ...
      try {
        ...
        const querySnapshot = await getDocs(
          query(userRef, where("friends", "array-contains", currentEmail))
        );
    ...
    <>
        ...
        <View className="absolute w-screen h-screen">
          ...
          {onFriendList && (
            <View className="absolute top-[20%] left-[10%] bg-white rounded-md p-3 z-10">
              <Text className="text-md font-semibold w-full text-center">
                Invite friends
              </Text>
              ...
              >
                ...
                {friends.length > 0 &&
                  friends.map((user, index) => (
                    <View
                      className="relative w-[220px] flex flex-row items-center space-x-3 bg-slate-100 p-2 rounded-md"
                      key={index}
                    >
                      <Image
                ...
                {messages.map((message, index) => (
                  <View
                    className={`flex flex-row space-x-3 items-center ${
                      message.fromEmail === currentEmail ? "justify-end" : ""
                    }`}
                    key={index}
                  >
                    {message.fromEmail !== currentEmail && (
                      <Image
                    ...
                    <View className="p-2 bg-slate-300 rounded-md">
                      ...
                      <Text className="text-[6px] text-gray-500">
                        {convertDate(parseInt(message.createdAt))}
                      </Text>
                    </View>
                  </View>
                ))}
      ...
      <View className={`absolute ${position()}`}>
        {userEmail ? (
          <View
            className={`relative ${
              no === 1 && "flex flex-row items-center space-x-2"
            }`}
          >
            ...
            <View>
              ...
              <View className="flex flex-row items-center space-x-1">
                ...
                <Image
                  source={require("../assets/stuff/coin.jpg")}
        ...
        {no !== 1 && userEmail && gameState === 1 && (
          <View className="absolute left-[60%]">
            ...
            <View className="absolute top-[20%] left-[10px] bg-white rounded-full w-[20px] h-[20px]">
              <Text className="text-center fonr-bold text-red-500">
                {remain}
              </Text>
            </View>
          </View>
